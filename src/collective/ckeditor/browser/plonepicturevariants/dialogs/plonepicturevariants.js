...
CKEDITOR.dialog.add( 'plonepicturevariants', function( editor ) {

	var features = editor.widgets.registered.image.features;
	features.variantSize = { requiredContent: 'img[data-scale]' };
	...
	var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
		...
		helpers = CKEDITOR.plugins.plonepicturevariants,
		...
		// Functions inherited from plonepicturevariants plugin.
		...
		contents: [
			{
				...
				elements: [
					...
					},
					{
						type: 'hbox',
						id: 'variantSize',
						// requiredContent: features.variantSize.requiredContent,
						children: [
							{
								id: 'variantSize',
								type: 'radio',
								...
								label: lang.variantSize,
								setup: function( widget ) {
									this.setValue( widget.data.variantSize );
								},
								commit: function( widget ) {
									widget.setData( 'variantSize', this.getValue() );
								}
							}
						]
					},
