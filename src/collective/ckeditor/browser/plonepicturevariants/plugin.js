...
( function() {
	...
	CKEDITOR.plugins.add( 'plonepicturevariants', {
		...
		init: function( editor ) {
			...
			if ( editor.plugins.detectConflict( 'plonepicturevariants', [ 'easyimage' ] ) ) {
				return;
			}

			// Adapts configuration from original image plugin. Should be removed
			// when we'll rename plonepicturevariants to image.
			var config = editor.config,
				lang = editor.lang.plonepicturevariants,
			...
			CKEDITOR.dialog.add( 'plonepicturevariants', this.path + 'dialogs/plonepicturevariants.js' );
		},
		...
		return {
			...
			data: function() {
				...
				this.parts.image.setAttributes( {
					...
					"data-picturevariant": this.data.variantSize
				} );
			...
			init: function() {
				var helpers = CKEDITOR.plugins.plonepicturevariants,
					...
					data = {
						...
						variantSize: image.getAttribute( 'data-picturevariant') || '',
	 ...
	 * @singleton
	 */
	CKEDITOR.plugins.plonepicturevariants = {
				...
				hasCaption:	function( shift, oldValue, newValue ) {
					...
					if ( newValue ) {
						...
						var figure = CKEDITOR.dom.element.createFromHtml( templateBlock.output( {
							...
							captionPlaceholder: editor.lang.plonepicturevariants.captionPlaceholder
						} ), doc );
				...
				link: function( shift, oldValue, newValue ) {
					if ( shift.changed.link ) {
						...
						if ( !newValue )
							...
							var attributes = CKEDITOR.plugins.plonepicturevariants.getLinkAttributesGetter()( editor, newValue );
	...
	function setupResizer( widget ) {
		...
		resizer.setAttribute( 'title', editor.lang.plonepicturevariants.resizer );
		...
		resizer.on( 'mousedown', function( evt ) {
			...
			function getMaxSize() {
				...
				natural = CKEDITOR.plugins.plonepicturevariants.getNatural( image );
	...
	function getWidgetAllowedContent( editor ) {
		var alignClasses = editor.config.image2_alignClasses,
			rules = {
				...
				img: {
					attributes: '!src,alt,data-picturevariant'
				},
				figure: {
